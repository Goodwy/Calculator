...
import dev.goodwy.math.helpers.Config
import dev.goodwy.math.helpers.MyWidgetProvider
import dev.goodwy.math.interfaces.CalculatorDao
    ...
    (0 until cnt).map { viewGroup.getChildAt(it) }
        .forEach {
            when (it) {
                ...
                is ImageView -> it.applyColorFilter(textColor)
...
fun Context.getStrokeColor(): Int {
    return if (isDynamicTheme()) {
        if (isSystemInDarkMode()) {
            resources.getColor(com.goodwy.commons.R.color.md_grey_800, theme)
        } else {
            resources.getColor(com.goodwy.commons.R.color.md_grey_400, theme)
        }
    } else {
        val lighterColor = getProperBackgroundColor().lightenColor()
        if (lighterColor == Color.WHITE || lighterColor == Color.BLACK) {
            resources.getColor(com.goodwy.commons.R.color.divider_grey, theme)
        } else {
            lighterColor
        }
    }
